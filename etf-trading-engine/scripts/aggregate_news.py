...
def main():
    ...
    if df.empty:
        pd.DataFrame(columns=["Date","Ticker","sentiment_raw","sentiment_z"]).to_csv(Path(a.outdir)/"sentiment.csv", index=False)
        pd.DataFrame(columns=["source","title","desc","link"]).to_csv(Path(a.outdir)/"news_articles.csv", index=False)
    ...
    out.to_csv(Path(a.outdir)/"sentiment.csv", index=False)
    df[["source","title","desc","link"]].to_csv(Path(a.outdir)/"news_articles.csv", index=False)
