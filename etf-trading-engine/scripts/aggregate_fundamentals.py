...
def fetch_fundamentals(tickers):
    ...
    R = pd.Series({k:v[0] for k,v in vals.items()})
    M = pd.Series({k:v[1] for k,v in vals.items()})
...
def main():
    ...
    ap.add_argument('--max_holdings', type=int, default=15)
    a = ap.parse_args()
    ...
    for etf in uni['Ticker'].unique().tolist():
        ...
        if not f: continue
        vect = np.array([f.get(t,0.0) for t in tickers], dtype=float)
        score = float(np.dot(weights, vect))
    ...
    feat.to_csv(Path(a.outdir)/'fundamentals.csv', index=False)
    print(f'fundamentals: wrote {len(feat)} rows')
